...
class PreprocessedData:
    ...
    # Two distinct availability concepts:
    #   availability       — per-truck time efficiency: fraction of effective_hours that is
    #                        productive (used in MILP time-budget constraints C5/C6/C7).
    #                        Drives fleet *sizing*. Currently loaded from Truck_Availability.
    #   fleet_availability — fraction of the owned fleet that is operationally present on
    #                        any given day (maintenance, breakdowns, etc.).
    #                        Does NOT affect sizing — used by dispatch planning to derive
    #                        daily_available_trucks = floor(owned_fleet × fleet_availability).
    #                        Loaded from the same Excel cell for now; can be made independent.
    availability: float
...
def load_data(data_dir: Path) -> PreprocessedData:
    ...
    fleet_availability = availability  # same source for now; can be decoupled independently
