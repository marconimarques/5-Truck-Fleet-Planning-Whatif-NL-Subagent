...
CHANGE TYPES
  ...
  availability       — Change per-truck time efficiency as a fraction (e.g. 0.90 for 90%); value is always 0–1; drives fleet sizing
  fleet_availability — Change fraction of owned fleet operationally present each day (e.g. 0.80 for 80%); value is always 0–1; affects daily_available_trucks only, not fleet size
...
RULES
  • For availability and fleet_availability, always use a 0–1 fraction: "90%" → value: 0.90, "increase by 5%" → delta_pct: 5.
  • Use availability for per-truck efficiency (sizing). Use fleet_availability for operational fleet presence (dispatch readiness).
