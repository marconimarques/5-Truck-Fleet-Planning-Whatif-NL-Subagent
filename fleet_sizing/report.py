...
def write_csv(results: list[ScopeResult], output_dir: Path, label: str = "output", description: str = "") -> Path:
    ...
    # Write MILP trip schedule (enriched) if present
    s43 = next((r for r in results if isinstance(r, Scope43Result) and r.trip_schedule), None)
    if s43 is not None:
        s41 = next((r for r in results if isinstance(r, Scope41Result)), None)
        cycle_map = (
            {(lr.cp_name, lr.terminal_name): lr.cycle_time_hours for lr in s41.lane_results}
            if s41 is not None else {}
        )
        eff = next(
            (lr.trips_per_truck_per_day * lr.cycle_time_hours
             for lr in s41.lane_results if lr.cycle_time_hours > 0),
            0.0,
        ) if s41 is not None else 0.0

        sched_path = _next_version(output_dir, f"{label}_milp_schedule")
        sched_fields = [
            "cp_name", "terminal_name", "day",
            "round_trips", "one_way_trips", "loaded_trips",
            "repo_trips", "payload_delivered_tons",
            "cycle_time_hours", "time_consumed_hours",
            "lane_trucks_needed", "daily_available_trucks",
        ]
        with open(sched_path, "w", newline="", encoding="utf-8") as sf:
            ...
            for entry in s43.trip_schedule:
                ...
                row["loaded_trips"] = entry.loaded_trips
                cycle = cycle_map.get((entry.cp_name, entry.terminal_name), 0.0)
                time_consumed = round(entry.loaded_trips * cycle, 2)
                row["cycle_time_hours"] = round(cycle, 3)
                row["time_consumed_hours"] = time_consumed
                row["lane_trucks_needed"] = round(time_consumed / eff, 2) if eff > 0 else ""
                row["daily_available_trucks"] = s43.daily_available_trucks
                sw.writerow(row)
